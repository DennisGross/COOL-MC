...
class SafeGym(gym.Env):
    ...
    def __init__(self, prism_file_path, constant_definitions, max_steps, wrong_action_penalty, reward_flag, permissive_input='', disabled_features='', abstraction_input=''):
        ...
        Args:
            prism_file_path ([type]): The Path to the PRISM environment
            constant_definitions ([type]): The constant definitions for the PRISM environment
            max_steps ([type]): The number of maximal steps in the environment
            wrong_action_penalty ([type]): The penalty for wrong actions
            reward_flag ([type]): If true, rewards, otherwise penalties
            permissive_input ([type]): The state variable ranges for the permissive model checking. Defaults to ''.
