...
class DDQNAgent(Agent):
    ...
    def step_learn(self):
        ...
        expected_q_values = self.q_eval(state_batch).gather(1, action_batch.unsqueeze(1))
        # Use current network to evaluate action argmax
        max_actions = self.q_eval.forward(n_state_batch).detach().argmax(dim=1).long()
        next_q_values = self.q_next.forward(n_state_batch).detach().gather(1, max_actions.unsqueeze(1))    
        next_q_values[done_batch] = 0
        q_target = reward_batch.to(device) + self.gamma * next_q_values
        loss = self.q_eval.loss(q_target, expected_q_values)
        loss.backward()
