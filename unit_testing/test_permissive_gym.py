...
def test_all_states_1d_0_2():
    ...
    all_states = PStateVariable.generate_all_states(env.storm_bridge.state_mapper.openai_state_variable_positions, np.array([[0]],dtype=np.int32), [pstate_variable])
...
def test_all_states_3d_1var():
    ...
    prism_file_path = os.path.join(command_line_arguments['prism_dir'], command_line_arguments['prism_file_path'])
    env = SafeGym(prism_file_path,command_line_arguments['constant_definitions'], command_line_arguments['max_steps'], command_line_arguments['wrong_action_penalty'], command_line_arguments['reward_flag'], command_line_arguments['permissive_input'], command_line_arguments['disabled_features'])
    all_states = PStateVariable.generate_all_states(env.storm_bridge.state_mapper.openai_state_variable_positions,fix_state, [pstate_variable])
    assert len(all_states) == 3
...
def test_all_states_3d_2vars2():
    ...
    prism_file_path = os.path.join(command_line_arguments['prism_dir'], command_line_arguments['prism_file_path'])
    env = SafeGym(prism_file_path,command_line_arguments['constant_definitions'], command_line_arguments['max_steps'], command_line_arguments['wrong_action_penalty'], command_line_arguments['reward_flag'], command_line_arguments['permissive_input'], command_line_arguments['disabled_features'])
    all_states = PStateVariable.generate_all_states(env.storm_bridge.state_mapper.openai_state_variable_positions,fix_state, [pstate_variable1])
    print(all_states)
